...
export const ErrorMessage = ({ error, onDismiss, className = '', showDismiss = true }) => {
  ...
  return (
    <div
      className={`flex-shrink-0 mx-4 p-3 bg-red-500 text-white rounded-lg text-sm relative ${className}`}
    >
            ...
            <div className="mt-3 p-3 bg-red-600 rounded text-xs">
              <div className="grid grid-cols-1 gap-2">
                {errorData.context.symbol && (
                  <div>
                    <strong>Symbol:</strong> {errorData.context.symbol}
                  </div>
                )}
                {errorData.context.date && (
                  <div>
                    <strong>Date:</strong> {errorData.context.date}
                  </div>
                )}
                {errorData.context.requestUrl && (
                  <div>
                    <strong>Request URL:</strong>{' '}
                    <span className="font-mono break-all">{errorData.context.requestUrl}</span>
                  </div>
                )}
                {errorData.context.httpStatusCode && errorData.context.httpStatusCode > 0 && (
                  <div>
                    <strong>HTTP Status:</strong> {errorData.context.httpStatusCode}
                  </div>
                )}
