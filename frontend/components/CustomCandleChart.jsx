...
export default function CustomCandleChart({
  ...
  // Use internal state for view management
  const viewState = internalViewState;
  const setViewState = setInternalViewState;
  const isDashboard = false; // This is a standalone page, not dashboard
  ...
  const drawChart = useCallback(() => {
    ...
    for (let i = 0; i < visibleCandles.length; i += xAxisStepSize) {
      ...
      if (candle && candle.time) {
        const x = xScale(visibleStart + i);
        // Time is already in HH:mm:ss format, just use it directly
        // For mobile, truncate to HH:mm
        const timeLabel = isMobile && candle.time.length > 5
          ? candle.time.substring(0, 5)
          : candle.time;
    ...
    if (showCrosshair && !isMobile && !isDragging) {
      ...
      const priceBoxX = width - padding.right + 5;
      const priceBoxY = mousePos.y - 10;

      ctx.fillStyle = colors.tooltip?.background || 'rgba(0, 0, 0, 0.9)';
      ctx.fillRect(priceBoxX, priceBoxY, priceBoxWidth, priceBoxHeight);
      ctx.strokeStyle = colors.tooltip?.border || colors.grid;
      ...
      if (hoveredCandle && hoveredCandle.time) {
        const timeText = hoveredCandle.time;
        const timeBoxWidth = 80;
        const timeBoxHeight = 20;
        const timeBoxX = mousePos.x - timeBoxWidth / 2;
        const timeBoxY = topPadding - 25;
        ...
        ctx.font = '12px -apple-system, BlinkMacSystemFont, sans-serif';
        ctx.textAlign = 'center';
        ctx.fillText(timeText, timeBoxX + timeBoxWidth / 2, timeBoxY + 14);
      }
    }
